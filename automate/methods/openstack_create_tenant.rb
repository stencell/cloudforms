...
def get_role_ids(openstack_keystone)
  ...
  openstack_keystone.list_roles[:body]["roles"].each { |role|
    log_and_update_message(:info, "role: #{role.inspect}")
...
def process_tags( category, category_description, single_value, tag, tag_description )
  ...
  log_and_update_message(:info, "Converted category name:<#{category_name}> Converted tag name: <#{tag_name}>")
  ...
  unless $evm.execute('tag_exists?', category_name, tag_name)
    log_and_update_message(:info, "Adding new tag <#{tag_name}> description <#{tag_description}> in Category <#{category_name}>")
    $evm.execute('tag_create', category_name, :name => tag_name, :description => "#{tag_description}")
  end
end

def update_status(options_hash)
  log_and_update_message(:info, "updating status: #{options_hash['status']}", true) unless options_hash['status'].blank?
  options_hash.each do |k,v|
    if @service
      ...
      unless @service.tagged_with?('tenant', options_hash['tenant_name'].to_s.downcase.gsub(/\W/, '_'))
        ...
        log_and_update_message(:info, "Tagged Service: #{@service.tags.inspect}")
      end
    end
  end
  $evm.set_state_var(:options_hash, options_hash)
  log_and_update_message(:info, "Workspace variable updated {:options_hash=>#{$evm.get_state_var(:options_hash)}")
end
...
begin
  ...
  log_and_update_message(:info, "Successfully created tenant #{new_tenant_hash.inspect}")
  ...
  log_and_update_message(:info, "openstack_admin: #{openstack_admin.inspect}")
  ...
  log_and_update_message(:info, "roles: #{net_tenant_roles.inspect}")
  ...
  log_and_update_message(:info, "User Roles Applied: #{openstack_keystone.list_roles_for_user_on_tenant(new_tenant_hash["id"], openstack_admin["id"]).inspect}")
