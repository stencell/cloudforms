...
def create_tenant_net(options_hash)
  ...
  log_and_update_message(:info, "added router: #{router["name"]}, network: #{tenant_net["name"]}, & subnet: #{tenant_subnet["name"]}")
end
...
begin
  ...
  options_hash['status'] = log_and_update_message(:info, "Network elements created for tenant: #{options_hash["tenant_name"]}")
