...
def set_quota(tenant='admin', cpu, mem, options_hash)
  ...
  log_and_update_message(:info, "Successfully connected to OpenStack #{tenant}")
  ...
  log_and_update_message(:info, "The ID of the newly created tenant #{tenant_name} is #{tenant_id}")
  new_quota = openstack_nova.update_quota(tenant_id, {"cores" => "#{cpu}", "ram" => "#{mem}"})
  log_and_update_message(:info, "Quota for tenant #{tenant_name} updated to #{new_quota.body['quota_set'].fetch("cores")} CPU 
...
begin
  ...
  log_and_update_message(:info, "tenant_name is #{options_hash["tenant_name"]}")
  options_hash.each { |k, v| log_and_update_message(:info, "options_hash contents - #{k}: #{v}")}
  ...
  options_hash['status'] = log_and_update_message(:info, "New CPU & MEM quota set for #{options_hash["tenant_name"]}")
