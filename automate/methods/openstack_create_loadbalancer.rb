...
def list_private_subnets(conn, tenant_id)
  @openstack_neutron.list_networks.body['networks'].select do |net|
    net
...
begin
  ...
  when 'service_template_provision_task'
    # Get task object from root
    @task = $evm.root['service_template_provision_task']
    @task.attributes.sort.each { |k,v| log(:info, "\t service_template_provision_task attribute - #{k}: #{v}") }
  ...
  # collect other global variables that we will need during processing
  @user = get_user
  ...
  uri = "/" if uri.nil?
  #we should get this as part of method if it is nil
  ...
  @openstack_neutron.list_networks.body['networks'].select do |net|
    net
  networks = openstack_neutron.list_networks.body['networks']
  log(:info, "Found Private Networks in tenant #{tenant.name}: #{networks.inspect}")

  # do we even need this??
  subnet = nil
  subnet = networks.each { |net| net['subnets'].first if net['subnets'].first == subnet_id }
  log(:info, "Found subnet #{subnet}")
  # end

  raise "Unable to locate subnet object from #{subnet_id}" if subnet.nil?

  log(:info, "Creating Load Balancer #{pool_name} in tenant #{tenant.name}", true)
