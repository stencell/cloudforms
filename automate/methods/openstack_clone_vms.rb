...
begin
  ...
  log_and_update_message(:info, "Inspecting existing_vms_hash: #{existing_vms_hash.inspect}")
  existing_vms_hash.keys.each { |k| existing_vms_hash[k.sub(/dialog_/, '')] = existing_vms_hash[k]; existing_vms_hash.delete(k) }
  log_and_update_message(:info, "New existing_vms_hash: #{existing_vms_hash.inspect}")
  ...
  log_and_update_message(:info, "Inspecting new_vms_hash: #{new_vms_hash.inspect}")
  new_vms_hash.keys.each { |k| new_vms_hash[k.sub(/dialog_/, '')] = new_vms_hash[k]; new_vms_hash.delete(k) }
  log_and_update_message(:info, "New new_vms_hash: #{new_vms_hash.inspect}")
